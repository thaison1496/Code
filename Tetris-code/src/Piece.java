...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for(int i=0; i<=3; i++) {
			...
			//if (body[i].y < skirt[body[i].x]) skirt[body[i].x] = body[i].y;
			if (body[i].x>width) width = body[i].x;
			if (body[i].y>height) height = body[i].y;
			
		//	System.out.print(body[i].x);
		//	System.out.print(" ");
		//	System.out.println(body[i].y);
		}
		width+=1; height+=1;
		skirt = new int[width];
	...
	*/
	...
	public Piece computeNextRotation() {
		int minx=4, miny=4;
		Piece p = new Piece(this.body);
		for(int i=0; i<=3; i++){
			p.body[i].x = 3 - this.body[i].y;
			p.body[i].y = this.body[i].x;
			if (p.body[i].x<minx) minx = p.body[i].x;
			if (p.body[i].y<miny) miny = p.body[i].y;
		}
		for(int i=0; i<=3; i++){
			p.body[i].x-=minx;
			p.body[i].y-=miny;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece currentP = root;
		while (true){
			Piece nextP = currentP.computeNextRotation();
			if (nextP.equals(root)) break;
			currentP.next = nextP;
			currentP = nextP;
		}
